...
Citizen.CreateThread(function()
	...
	TriggerEvent('chat:addSuggestion', '/repair', 'Temporary fixes vehicle', {})
...
RegisterNetEvent('ns_vehfailure:repair')
AddEventHandler('ns_vehfailure:repair', function()
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				exports['progressBars']:startUI(3500, "Temporary Fixing Vehicle")
                Citizen.Wait(3500)
				healthEngineLast=cfg.cascadingFailureThreshold +5
				healthPetrolTankLast=250.0
				SetVehicleEngineOn(vehicle, true, false )
				SetVehicleOilLevel(vehicle,(GetVehicleOilLevel(vehicle)/3)-0.5)
				exports['ns_notify']:sendNotification("Temporarily Fixed Vehicle, Now Get to a Mechanic!", {type='success', horizontal="right", variant='filled', duration=3500})
				fixMessagePos = fixMessagePos + 1
				if fixMessagePos > repairCfg.fixMessageCount then fixMessagePos = 1 end
			else
				exports['ns_notify']:sendNotification("Your vehicle was too badly damaged. Unable to temporarily fix it!", {type='error', horizontal="right", variant='filled', duration=3500})
			end
		else
			exports['ns_notify']:sendNotification("Your vehicle doesnt need to be fixed yet", {type='error', horizontal="right", variant='filled', duration=3500})
			noFixMessagePos = noFixMessagePos + 1
			if noFixMessagePos > repairCfg.noFixMessageCount then noFixMessagePos = 1 end
		end
	else
		exports['ns_notify']:sendNotification("You must be in a vehicle to be able to temporarily fix it!", {type='info', horizontal="right", variant='filled', duration=2500})
	end
end)

RegisterNetEvent('ns_vehfailure:notAllowed')
AddEventHandler('ns_vehfailure:notAllowed', function()
		...
		if isPedDrivingAVehicle() then
			...
			if pedInSameVehicleLast == true then
				...
				if healthEngineCurrent ~= 1000.0 or healthBodyCurrent ~= 1000.0 or healthPetrolTankCurrent ~= 1000.0 then
					...
					-- Prevent Explosions
					if cfg.compatibilityMode == false and healthPetrolTankCurrent < 1 then
